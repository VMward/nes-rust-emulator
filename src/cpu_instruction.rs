...
impl Instruction {
    // Convert rom code to instructions
...
impl Cpu {
    ...
    // ret: (Operand (subtracted immediate value or address, number of clocks), data)
    fn fetch_args(&mut self, system: &mut System, mode: AddressingMode) -> (Operand, u8) {
        match mode {
            ...
            // pull back the data from the returned address. May not be used
            _ => {
    ...
    pub fn step(&mut self, system: &mut System) -> u8 {
        ...
        match opcode {
            /* *************** binary op ***************  */
            // The result is stored in the a register, so the operand address is not used.
            Opcode::ADC => {
            ...
            Opcode::ASL => {
                ...
                if mode == AddressingMode::Accumulator {
                    ...
                    // print calculation result to the original address
            ...
            /* *************** inc/dec op ***************  */
            // Danger: Do not use accumulator, version using x, y registers is Implied
            Opcode::INC => {
                let (Operand(addr, cyc), arg) = self.fetch_args(system, mode);
            ...
            Opcode::JSR => {
                ...
                // get address where the opcode was (advance 3 with opcode, operand fetch)                let opcode_addr = inst_pc;
            ...
            Opcode::TXS => {
                // High-order byte of sp is fixed to 0x01
            ...
            Opcode::BIT => {
                ...
                // Need non-destructive read, read by yourself without fetch_args ...
                let Operand(addr, cyc) = self.fetch_operand(system, mode);
                let arg = system.read_u8(addr, true); // safe read
            ...
            Opcode::ANC => {
// Immediate only, A = A & #IMM, Carry copies Negative of the previous state                debug_assert!(mode == AddressingMode::Immediate);
                let (Operand(_, cyc), arg) = self.fetch_args(system, mode);
            ...
            Opcode::ARR => {
            // Immediate only, Carry = bit6, V = bit6 xor bit5                debug_assert!(mode == AddressingMode::Immediate);
                let (Operand(_, cyc), arg) = self.fetch_args(system, mode);
            ...
            Opcode::AXS => {
                // Immediate only, X = (A & X)-Updated #IMM, NZC
                // without borrow, so ignore the c flag when subtractingdebug_assert!(mode == AddressingMode::Immediate);
                let (Operand(_, cyc), arg) = self.fetch_args(system, mode);
